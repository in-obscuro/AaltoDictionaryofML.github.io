% !TeX spellcheck = de_DE
 wuhtutuethwoutwoeutpweuothwpeuth
...
Im Kontext von \gls{ml} ist ein besonders wichtiges Beispiel eines \gls{vectorspace}
der \gls{euclidspace} $\mathbb{R}^{\nrfeatures}$, wobei $\nrfeatures \in \mathbb{N}$
die (endliche) Dimension des Raumes ist. Ein Vektor $\vx \in \mathbb{R}^{\nrfeatures}$
kann als Liste oder als eindimensionales Array reeller Zahlen dargestellt werden, d.,h.
$x_1, \ldots, x_{\nrfeatures}$ mit $x_\featureidx \in \mathbb{R}$ für
$\featureidx = 1, \ldots, \nrfeatures$. Der Wert $x_\featureidx$ ist der $\featureidx$-te
Eintrag des Vektors $\vx$.
Es kann auch nützlich sein, einen Vektor $\vx \in \mathbb{R}^{\nrfeatures}$ als \gls{function}
aufzufassen, die jedem Index $\featureidx \in {1, \ldots, \nrfeatures}$
einen Wert $x_\featureidx \in \mathbb{R}$ zuordnet, also $\vx: \featureidx \mapsto x_\featureidx$.
Diese Perspektive ist insbesondere für das Studium von \glspl{kernelmethod} hilfreich.
\begin{figure}[H]
% Left: Stem plot
...
\end{tikzpicture}
\end{minipage}
\caption{Zwei äquivalente Sichtweisen eines Vektors $\vx= \big( 2, -1, 3, 0, -2, 1 \big)^{T} \in \mathbb{R}^{6}$.
(a) Als numerisches Array. (b) Als \gls{map} $\featureidx \mapsto x_\featureidx$.}
\label{fig:vector-function-dual_dict}
\end{figure}
Siehe auch: \gls{vectorspace}, \gls{euclidspace}, \gls{linearmap}.},
first={Vektor},
type={math},
firstplural={Vektoren},
plural={Vektoren},
text={Vektor}
}
...
{name={Vektorraum},
description={Ein\index{vector space} \gls{vector}raum $\mathcal{V}$ (auch linearer Raum genannt)
ist eine Menge von Elementen, genannt \glspl{vector}, zusammen mit zwei Operationen:
Addition (bezeichnet durch $\vv+\vw$) zweier Vektoren $\vv,\vw$ und Multiplikation
(bezeichnet durch $c\cdot \vv$) eines Vektors $\vv$ mit einem Skalar $c$, der zu einem
Zahlenkörper gehört (eine typische Wahl für diesen Körper ist $\mathbb{R}$).
Die definierende Eigenschaft eines Vektorraums ist, dass er unter diesen Operationen abgeschlossen ist, d.,h.,
\begin{itemize}
\item Falls $\vv, \vw \in \mathcal{V}$, dann gilt $\vv + \vw \in \mathcal{V}$.
\item Falls $\vv \in \mathcal{V}$ und $c \in \mathbb{R}$, dann gilt $c \vv \in \mathcal{V}$.
\item Insbesondere gilt $\mathbf{0} \in \mathcal{V}$.
\end{itemize}
\begin{figure}[ht]
\centering
...
% (figure unchanged)
\end{tikzpicture}
\caption{Ein Vektorraum $\mathcal{V}$ ist eine Menge von Vektoren, sodass Skalieren
und Addieren stets wieder einen Vektor in $\mathcal{V}$ ergibt. In \gls{ml}-Anwendungen
werden Vektoren verwendet, um \glspl{datapoint} (oder deren \glspl{featurevec}) sowie
Invarianzen (oder Symmetrien) von \glspl{model} darzustellen.}
\label{fig:vector-ops_dict}
\end{figure}
Ein typisches Beispiel für einen Vektorraum ist der \gls{euclidspace} $\mathbb{R}^n$, der in
\gls{ml} weit verbreitet zur Darstellung von \glspl{model} und \glspl{dataset} verwendet wird.
\
Siehe auch: \gls{vector}, \gls{euclidspace}, \gls{linmodel}, \gls{linearmap}.},
first={vector space},
type=math,
text={vector space}
}
...
{name={stochastic},
description={Wir bezeichnen eine \index{stochastic} Methode als stochastisch, wenn sie eine
zufällige Komponente beinhaltet oder durch probabilistische Gesetzmäßigkeiten bestimmt ist.
\Gls{ml}-Methoden nutzen Zufall, um die rechnerische Komplexität zu reduzieren (z.,B. siehe \gls{stochGD})
oder um \gls{uncertainty} in \glspl{probmodel} abzubilden.
\
Siehe auch: \gls{stochGD}, \gls{uncertainty}, \gls{probmodel}.},
first={stochastic},
text={stochastic}
}

\newglossaryentry{stochproc}
{name={stochastischer Prozess},
description={Ein \gls{stochastic}er Prozess\index{stochastic process} ist eine Familie von
\glspl{rv}, die auf einem gemeinsamen \gls{probspace} definiert sind und durch eine Menge
$\mathcal{I}$ indiziert werden \cite{papoulis,GrayProbBook,Brockwell91}. Die Indexmenge
$\mathcal{I}$ repräsentiert typischerweise Zeit oder Raum und erlaubt es, zufällige Phänomene
zu modellieren, die sich zeitlich oder räumlich entwickeln – etwa Sensorausrauschen oder
finanzielle Zeitreihen. Stochastische Prozesse sind jedoch nicht auf zeitliche oder räumliche
Kontexte beschränkt. Beispielsweise können zufällige \glspl{graph} wie das \gls{ergraph} oder das
\gls{sbm} ebenfalls als stochastische Prozesse angesehen werden. Hier besteht die Indexmenge
$\mathcal{I}$ aus Knotenpaaren, welche \glspl{rv} indizieren, deren Werte das Vorhandensein oder
das Gewicht einer Kante zwischen zwei Knoten kodieren. Darüber hinaus treten \gls{stochastic}e
Prozesse natürlich auch in der Analyse von \glspl{stochalgorithm} auf, wie etwa \gls{stochGD},
die eine Folge von \glspl{rv} konstruieren.
\
Siehe auch: \gls{rv}, \gls{sbm}, \gls{stochGD}, \gls{uncertainty}, \gls{probmodel}.},
...
description={Die charakteristische \gls{function}\index{characteristic function}
einer reellwertigen \gls{rv} $x$ ist die \gls{function} \cite[Sec. 26]{BillingsleyProbMeasure}

ϕx(t)\defeq\expectexp⁡ (jtx)\mboxwithj=−1.
ϕ
x
	​

(t)\defeq\expectexp(jtx)\mboxwithj=
−1

​

.
Die charakteristische \gls{function} bestimmt die \gls{probdist} von $x$ eindeutig.
\
Siehe auch: \gls{rv}, \gls{probdist}.},
...
{name={Entropie},
	description={Entropie\index{entropy} quantifiziert die \gls{uncertainty} oder Unvorhersehbarkeit, 
		die mit einer \gls{rv} verbunden ist \cite{coverthomas}. 
		Für eine diskrete \gls{rv} $x$, die Werte in einer endlichen Menge 
		$\mathcal{S} = \{x_1, \,\ldots, \,x_n\}$ annimmt und eine \gls{probability}-Massen-\gls{function} 
		$p_i \defeq \prob{x = x_i}$ besitzt, ist die Entropie definiert als
		\[
		H(x) \defeq -\sum_{i=1}^n p_i \log p_i.
		\]
		Die Entropie ist maximal, wenn alle Ausgänge gleich wahrscheinlich sind, und minimal (d.\,h. null), 
		wenn das Ergebnis deterministisch ist. Eine \gls{generalization} des Entropiebegriffs für 
		continuous-valued \glspl{rv} ist die \gls{diffentropy}. 
		\\
		Siehe auch: \gls{uncertainty}, \gls{probmodel}.},
	first={Entropie},
	text={Entropie}
}

\newglossaryentry{diffentropy}
{name={differentielle Entropie},
description={Für\index{differentielle Entropie} eine reellwertige \gls{rv}
$\featurevec \in \mathbb{R}^{\nrfeatures}$ mit einer \gls{pdf} $p(x)$ ist die differentielle
\gls{entropy} definiert als \cite{coverthomas}

h(\featurevec)\defeq−∫p(\featurevec)log⁡p(\featurevec) d\featurevec.
h(\featurevec)\defeq−∫p(\featurevec)logp(\featurevec)d\featurevec.
Die differentielle \gls{entropy} kann negativ sein und besitzt einige Eigenschaften der
\gls{entropy} für diskrete \glspl{rv} nicht, etwa Invarianz unter Variablentransformationen
\cite{coverthomas}. Unter allen \glspl{rv} mit gegebenem \gls{mean} $\meanvecgeneric$ und
\gls{covmtx} $\covmtxgeneric$ wird $h(\featurevec)$ durch
$\featurevec \sim \mvnormal{\meanvecgeneric}{\covmtxgeneric}$ maximiert.
\
Siehe auch: \gls{uncertainty}, \gls{probmodel}.},
first={differentielle Entropie},
text={differentielle Entropie}
}

	\newglossaryentry{minimum}
{name=Minimum,
 description={Bei einer gegebenen Menge and reeller Zahlen, ist
  das Minimum \index{minimum} das kleinste dieser Zahlen.},
 first={Minimum},
 text={Minimum},
  plural={Minima}
}
...
\newglossaryentry{map}
{name={Abbildung}, 
	description={Wir\index{map} verwenden den Begriff Abbildung als Synonym für \gls{function}.
		\\
		Siehe auch: \gls{function}.},
	first={Abbildung},
	firstplural={Abbildungen},	
	plural={Abbildungen},
	text={Abbildung}
}
...
{name={large language model (LLM)},
 description={Ein\index{large language model (LLM)} LLM ist ein Oberbegriff für \gls{ml}-Methoden, 
               die hochdimensionale \gls{ml} \glspl{model} (mit Milliarden von \glspl{modelparam}) 
               verwenden und auf großen Text-\gls{data}-Sammlungen trainiert werden. 
               LLMs werden verwendet, um Sequenzen von \glspl{token} zu analysieren oder zu generieren, 
               die Text-\gls{data} darstellen. Viele aktuelle LLMs verwenden eine Variante eines 
               \gls{transformer}, der mittels selbstüberwachtem Lernen trainiert wird:  
               Das Training basiert auf der Aufgabe, einige absichtlich aus einem großen Textkorpus 
               entfernte Wörter vorherzusagen. Auf diese Weise können \glspl{labeled datapoint} 
               einfach konstruiert werden, indem einige Wörter eines gegebenen Textes als \glspl{label} 
               und die verbleibenden Wörter als \glspl{feature} der \glspl{datapoint} ausgewählt werden. 
               Diese Konstruktion erfordert sehr wenig menschliche Aufsicht und ermöglicht die 
               Erstellung von ausreichend großen \glspl{trainset} für LLMs.\\
               Siehe auch: \gls{nlp}, \gls{token}, \gls{transformer}.}, 
  first = {LLM}, 
  firstplural={LLMs}, 
  text = {LLM}, 
  plural = {LLMs} 
...
{name={Optimierungsproblem}, 
	description={Ein\index{optimization problem} Optimierungsproblem ist eine mathematische 
		   Struktur, bestehend aus einer \gls{objfunc} $f: \mathcal{U} \rightarrow \mathcal{V}$, 
		   definiert über eine Optimierungsvariable $\weights \in \mathcal{U}$, zusammen mit einer 
		   zulässigen Menge $\mathcal{W} \subseteq \mathcal{U}$. Der Zielbereich $\mathcal{V}$ 
		   wird als geordnet angenommen, das heißt, für zwei Elemente $\mathbf{a}, \mathbf{b} \in \mathcal{V}$ 
		   lässt sich bestimmen, ob $\mathbf{a} < \mathbf{b}$, $\mathbf{a} = \mathbf{b}$ oder 
		   $\mathbf{a} > \mathbf{b}$ gilt. Ziel der Optimierung ist es, jene Werte $\weights \in \mathcal{W}$ 
		   zu finden, für die das Ziel $f(\weights)$ extremal ist—d.h. minimal oder maximal \cite{BoydConvexBook}, \cite{BertsekasNonLinProgr}, \cite{nesterov04}.
		   \\
		   Siehe auch: \gls{objfunc}.},
	first={Optimierungsproblem},
	firstplural={Optimierungsprobleme}, 
	plural={Optimierungsprobleme}, 
	text={Optimierungsproblem}
}

\newglossaryentry{optmethod}
{name={Optimierungsmethode},
	description={Eine\index{optimization method} Optimierungsmethode ist ein \gls{algorithm}, der 
		eine Darstellung eines \gls{optproblem} einliest und eine (approximative) Lösung 
		als Ergebnis liefert \cite{BoydConvexBook}, \cite{BertsekasNonLinProgr}, \cite{nesterov04}.
		 \\
		 Siehe auch: \gls{algorithm}, \gls{optproblem}.},
	first={Optimierungsmethode},
	firstplural={Optimierungsmethoden}, 
	plural={Optimierungsmethoden}, 
	text={Optimierungsmethode}
}
...
{name={Fixpunktiteration},
description={Eine\index{Fixpunktiteration} Fixpunktiteration ist ein iteratives Verfahren zur Lösung eines gegebenen \gls{optproblem}. Sie konstruiert eine Folge $\weights^{(0)}, \weights^{(1)}, \ldots$ durch wiederholte Anwendung eines Operators $\fixedpointop$, d.,h.,
\begin{equation}
\label{equ_def_fixed_point_dict}
\weights^{(\iteridx+1)} = \fixedpointop \weights^{(\iteridx)} \mbox{, für } \iteridx=0, 1, \ldots.
\end{equation}
Der Operator $\fixedpointop$ wird so gewählt, dass jeder seiner Fixpunkte eine Lösung $\widehat{\weights}$ des gegebenen \gls{optproblem} ist. Beispielsweise stimmen bei einer \gls{differentiable} und \gls{convex}en \gls{function} $f(\weights)$ die Fixpunkte des Operators $\fixedpointop: \weights \mapsto \weights - \nabla f(\weights)$ mit den Minimierern von $f(\weights)$ überein. Im Allgemeinen existieren für ein gegebenes \gls{optproblem} mit Lösung $\widehat{\weights}$ viele verschiedene Operatoren $\fixedpointop$, deren Fixpunkte $\widehat{\weights}$ sind.
Offensichtlich sollten wir einen Operator $\fixedpointop$ in \eqref{equ_def_fixed_point_dict} verwenden, der den Abstand zu einer Lösung reduziert, sodass
\begin{equation}
\nonumber
...
\end{equation}
Daher verlangen wir, dass $\fixedpointop$ zumindest nicht-expansiv ist, d.,h. die Iteration \eqref{equ_def_fixed_point_dict} sollte nicht zu schlechteren \glspl{modelparam} führen, die einen größeren Abstand zu einer Lösung $\widehat{\weights}$ besitzen.
Zudem sollte jede Iteration \eqref{equ_def_fixed_point_dict} auch Fortschritt erzielen, d.,h. den Abstand zu einer Lösung $\widehat{\weights}$ verringern. Dieses Erfordernis lässt sich mithilfe des Begriffs eines \gls{contractop} präzisieren \cite{Bauschke:2017}, \cite{fixedpoinIsta}.
Der Operator $\fixedpointop$ ist ein \gls{contractop}, wenn für ein $\contractfac \in [0,1)$ gilt:
\begin{equation}
\nonumber
\normgeneric{ \fixedpointop \weights!-!\fixedpointop \weights'}{2} \leq \contractfac \normgeneric{\weights!-!\weights'}{2} \mbox{ für alle } \weights,\weights'.
\end{equation}
Für einen \gls{contractop} $\fixedpointop$ erzeugt die Fixpunktiteration \eqref{equ_def_fixed_point_dict} eine Folge $\weights^{(\iteridx)}$, die sehr schnell konvergiert. Insbesondere gilt \cite[Th. 9.23]{RudinBookPrinciplesMatheAnalysis},
\begin{equation}
\nonumber
\normgeneric{ \weights^{(\iteridx)} - \widehat{\weights}}{2} \leq \contractfac^{\iteridx} \normgeneric{ \weights^{(0)} - \widehat{\weights}}{2}.
\end{equation}
Hier ist $\normgeneric{ \weights^{(0)} - \widehat{\weights}}{2}$ der Abstand zwischen der Initialisierung $\weights^{(0)}$ und der Lösung $\widehat{\weights}$.
Es stellt sich heraus, dass eine Fixpunktiteration \eqref{equ_def_fixed_point_dict} mit einem fest nicht-expansiven Operator $\fixedpointop$ garantiert zu einem Fixpunkt von $\fixedpointop$ konvergiert \cite[Cor. 5.16]{Bauschke:2017}.
Abbildung \ref{fig_examples_nonexp_dict} zeigt Beispiele für einen fest nicht-expansiven Operator, einen nicht-expansiven Operator sowie einen \gls{contractop}. Alle diese Operatoren sind im eindimensionalen Raum $\mathbb{R}$ definiert.
Ein weiteres Beispiel für einen fest nicht-expansiven Operator ist der \gls{proxop} einer \gls{convex}en \gls{function} \cite{Bauschke:2017}, \cite{ProximalMethods}.
...
\end{tikzpicture}
\end{center}
\caption{Beispiel eines nicht-expansiven Operators $\fixedpointop^{(1)}$, eines fest nicht-expansiven Operators $\fixedpointop^{(2)}$ und eines \gls{contractop} $\fixedpointop^{(3)}$. \label{fig_examples_nonexp_dict}}
\end{figure}
Siehe auch: \gls{optproblem}, \gls{differentiable}, \gls{convex} \gls{function}, \glspl{modelparam}, \gls{contractop}, \gls{proxop}.},
first={Fixpunktiteration},
text={Fixpunktiteration},
firstplural={Fixpunktiterationen},
plural={Fixpunktiterationen}
}


\newglossaryentry{ergraph}
{name={Erd\H{o}s-Rényi-Graph (ER-Graph)},
description={Ein ER-\gls{graph} ist ein \gls{probmodel} für \glspl{graph}, definiert über einer gegebenen Knotenmengen $\nodeidx=1, \ldots, \nrnodes$. Eine Möglichkeit, den ER-\gls{graph} zu definieren, besteht in der Verwendung der Menge von \gls{iid} binären \glspl{rv} $b^{(\edge{\nodeidx}{\nodeidx'})} \in {0,1}$ für jedes Paar verschiedener Knoten $\nodeidx, \nodeidx'$. Eine konkrete \gls{realization} eines ER-\gls{graph} enthält eine Kante $\edge{\nodeidx}{\nodeidx'}$ genau dann, wenn $b^{(\edge{\nodeidx}{\nodeidx'})}=1$ gilt. Der ER-\gls{graph} wird durch die Anzahl $\nrnodes$ der Knoten und die \gls{probability} $\prob{b^{(\edge{\nodeidx}{\nodeidx'})}=1}$ parametrisiert. \
Siehe auch: \gls{graph}, \gls{probmodel}, \gls{iid}, \gls{rv}, \gls{realization}, \gls{probability}.},
first={Erd\H{o}s-Rényi-(ER-)Graph},
text={ER-Graph}
}

\newglossaryentry{attack}
{name={Angriff},
description={Ein Angriff\index{Angriff} auf ein \gls{ml}-System bezeichnet eine gezielte Handlung – entweder aktiv oder passiv –, die die Integrität, Verfügbarkeit oder Vertraulichkeit des Systems beeinträchtigt.
Aktive Angriffe beinhalten die Manipulation von Komponenten wie \glspl{dataset} (durch \gls{datapoisoning}) oder von Kommunikationsverbindungen zwischen \glspl{device} innerhalb einer \gls{ml}-Anwendung.
Passive Angriffe, wie \glspl{privattack}, zielen darauf ab, \glspl{sensattr} zu rekonstruieren, ohne das System zu verändern.
Je nach Ziel unterscheiden wir zwischen \glspl{dosattack}, \gls{backdoor}-Angriffen und \glspl{privattack}.
\
Siehe auch: \gls{datapoisoning}, \gls{privattack}, \gls{sensattr}, \gls{dosattack}, \gls{backdoor}.},
plural={Angriffe},
first={Angriff},
firstplural={Angriffe},
text={Angriff}
}

\newglossaryentry{privattack}
{name={Privacy-Angriff},
description={Ein Privacy-\gls{attack}\index{Privacy-Angriff} auf ein \gls{ml}-System zielt darauf ab, \glspl{sensattr} von Individuen zu rekonstruieren, indem partieller Zugriff auf ein trainiertes \gls{ml}-\gls{model} ausgenutzt wird.
Eine Form eines Privacy-\gls{attack} ist \gls{modelinversion}.\
Siehe auch: \gls{attack}, \gls{sensattr}, \gls{modelinversion}, \gls{trustAI}, \gls{gdpr}.},
plural={Privacy-Angriffe},
first={Privacy-Angriff},
firstplural={Privacy-Angriffe},
text={Privacy-Angriff}
}
		...
		\newglossaryentry{explainability}
			{name={Erklärbarkeit},description=
				{Wir \index{explainability} definierien die subjektive Erklärbarkeit einer \gls{ml}- Methode als Maß für den Grad der 
						Simulierbarkeit \cite{Colin:2022aa} der von einem \gls{ml}- System gelieferten \glspl{prediction} fü einen menschlichen Nutzer. 
						Quantiative Maße der subjektiven Erllärbarkeit eines trainierten \gls{model}s können konstruiert werden, in dem man dessen 
						\glspl{prediction} mit den \glspl{prediction} vergleicht, die von einem Nutzer durch ein \gls{testset} 
						bereitgestellt werden \cite{Colin:2022aa}, \cite{Zhang:2024aa}. 
						...
						\gls{model}s über die konditionale (bzw. differentielle) Entropie seiner  \glspl{prediction}, bei gegebenen \glspl{prediction}
						 der Nutzer messen \cite{JunXML2020}, \cite{Chen2018}. 
					},
